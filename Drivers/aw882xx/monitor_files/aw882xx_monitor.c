...
int aw882xx_monitor_work_func(void *dev)
{
	int ret = 0;
	...
	aw_dev_dbg(aw_dev->dev_index,
		"spk_mode:%d, monitor_status:%d, monitor_switch:%d", aw_dev->spk_mode,
		monitor_cfg->monitor_status, monitor_cfg->monitor_switch);

	if ((aw_dev->spk_mode == AW_SPK_MODE) &&
...
void aw882xx_monitor_start(void *dev)
{
	int ret = -1;
	struct aw_device *aw_dev = dev;
	...
	ret = aw882xx_monitor_work_func((void *)aw_dev);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev_index, "monitor work failed");
	}
}

void aw882xx_monitor_stop(void *dev)
{
	struct aw_device *aw_dev = dev;
	aw_dev_info(aw_dev->dev_index, "enter");
...
int aw882xx_monitor_init(void *dev)
{
	int ret= -1;
	struct aw_device *aw_dev = dev;
...
void aw882xx_monitor_set_handle (void *dev)
{
	struct aw_device *aw_dev = dev;
...
void aw882xx_monitor_deinit(void *dev)
{
	struct aw_device *aw_dev = dev;
